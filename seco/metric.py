...
class AccuracyLogger():
    ...
    def __init__(self, idx2label_dict):
        ...
        self.confusionmatrices = torch.zeros(2, self.NUM_CLASSES, self.NUM_CLASSES)
    ...
    def update(self, predictions, groundtruth, bbox_area=None):
        """
        Parameters:
        ...
        if bbox_area is None:
            ...
            assert len(predictions) == len(groundtruth) and len(predictions) == len(bbox_area), "Predictions and groundtruth should be of the same length."

            for pred, gt, bbox in zip(predictions, groundtruth, bbox_area):
                self.confusionmatrix[gt, pred] += 1
                if bbox < 0.005:
                    self.confusionmatrices[0, gt, pred] += 1  
                else:
                    self.confusionmatrices[1, gt, pred] += 1  
    ...
    def accuracy(self, confusionmatrix=None):
        ...
        """
        confusionmatrix = self.confusionmatrix if confusionmatrix is None else confusionmatrix
        return (confusionmatrix.diag()/(confusionmatrix.sum(dim=1) + 1e-7)).mean().item()

    def class_accuracies(self, confusionmatrix=None):
        """
        returns a dictionary mapping class ind to accuracy.
        """
        confusionmatrix = self.confusionmatrix if confusionmatrix is None else confusionmatrix
        return {idx: acc.item() for idx, acc in enumerate(confusionmatrix.diag()/(confusionmatrix.sum(dim=1) + 1e-7))}

    def named_class_accuarcies(self, confusionmatrix=None):
        """
        returns a dictionary mapping class name to accuracy.
        """
        confusionmatrix = self.confusionmatrix if confusionmatrix is None else confusionmatrix
        return {self.idx2label[idx]: acc.item() for idx, acc in enumerate(confusionmatrix.diag()/(confusionmatrix.sum(dim=1) + 1e-7))}

    def normalized_confusionmatrix(self, confusionmatrix=None):
        """
        ...
        return (confusionmatrix.transpose(0,1) / (confusionmatrix.sum(dim=1) + 1e-7)).transpose(0,1)
    ...
    def items_without_threshold(self):
        return {"total_accuracy": self.accuracy(),
                "named_class_accuracies": self.named_class_accuarcies(),               
                }.items(),

    def items(self):
        return {"total_accuracy": self.accuracy(),
                "<0.005_accuracy": self.accuracy(self.confusionmatrices[0]),
                ">=0.005_accuracy": self.accuracy(self.confusionmatrices[1]),
                "named_class_accuracies": self.named_class_accuarcies(),
                "<0.005_named_class_accuarcies": self.named_class_accuarcies(self.confusionmatrices[0]),
                ">=0.005_named_class_accuarcies": self.named_class_accuarcies(self.confusionmatrices[1]),                
                }.items(),
    def bbox_items(self):
        return {"<0.005_accuracy": self.accuracy(self.confusionmatrices[0]),
                ">=0.005_accuracy": self.accuracy(self.confusionmatrices[1]),
                "<0.005_named_class_accuarcies": self.named_class_accuarcies(self.confusionmatrices[0]),
                ">=0.005_named_class_accuarcies": self.named_class_accuarcies(self.confusionmatrices[1]),                
                }       
